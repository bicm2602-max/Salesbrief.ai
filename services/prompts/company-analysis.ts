...
export function buildCompanyAnalysisPrompt(content: string, prospect?: ProspectContext) {
  ...
  return [
    ...
    "Use the schema: { companyName, industry, description, companySize, targetCustomers, products, services, painPoints, opportunities, buyingSignals, salesScore, confidenceScore, recommendedOffer, email, linkedin, coldCall, followUp, objections, summary, outreachVariants }. outreachVariants must be { emailShort, emailConsultative, linkedinShort, linkedinConversational }. objections must be an array of plain strings, never objects.",
