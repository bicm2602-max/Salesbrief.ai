...
export function PricingPreview() {
  const router = useRouter(); const searchParams = useSearchParams(); const selected = searchParams.get("plan"); const selectedPlan = selected === "starter" || selected === "pro" ? selected : null; const [loading, setLoading] = React.useState<string | null>(null); const [error, setError] = React.useState<string | null>(null); const [notice, setNotice] = React.useState<string | null>(null); const [subscription, setSubscription] = React.useState<{ authenticated: boolean; plan: "free" | "starter" | "pro" | "business" } | null>(null);
  ...
  async function choose(plan: "starter" | "pro" | "business") {
    ...
    try {
      ...
      if (result.ok && result.action === "updated") { setNotice(`Your subscription has been upgraded to ${result.plan.charAt(0).toUpperCase() + result.plan.slice(1)}.`); refreshSubscription(); router.refresh(); return; }
