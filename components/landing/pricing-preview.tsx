...
export function PricingPreview({ initialSubscription }: { initialSubscription: PricingSubscription }) {
  ...
  const choose = React.useCallback(async (plan: "starter" | "pro" | "business") => {
  ...
  const continuedPlan = React.useRef<string | null>(null);
  React.useEffect(() => {
    if (!selectedPlan || !subscription.authenticated || continuedPlan.current === selectedPlan) return;
    continuedPlan.current = selectedPlan;
    window.history.replaceState(null, "", `${window.location.pathname}#pricing`);
    void choose(selectedPlan);
  }, [choose, selectedPlan, subscription.authenticated]);
